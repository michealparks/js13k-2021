...
// Oscillators
let osc_sin = (value) => Math.sin(value * 6.283184),
    osc_saw = (value) => 2 * (value % 1) - 1,
    osc_square = (value) => (value % 1) < 0.5 ? 1 : -1,
    getnotefreq = (n) => 0.003959503758 * (2 ** ((n - 128) / 12)),
...
createNote = (instr, n, rowLen) => {
  let osc1 = mOscillators[instr.i[0]],
      ...
      noteBuf = new Int32Array(attack + sustain + release),
      // Re-trig oscillators
      c1 = 0, c2 = 0,
      // Local variables.
      j, j2, e, t, rsample, o1t, o2t;
...
mSong, mLastRow, mCurrentCol, mNumWords, mMixBuf;

//--------------------------------------------------------------------------
// Initialization
//--------------------------------------------------------------------------

export let initSong = (song) => {
    // Define the song
    mSong = song
...
export let generateSong = () => {
    ...
    // Put performance critical items in local variables
        chnBuf = new Int32Array(mNumWords),
